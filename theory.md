...
This document provides a self-contained and didactic overview of the theory behind the code in this repository.
* * *
...
The long - $\tau$ decay of $G(\tau)$ encodes the **energy gaps** of the quantum spectrum. In particular, it allows us to estimate the gap between the first excited state and the ground state:
...
Intuitively, quantum statistics at temperature $T$ becomes a classical statistical mechanics problem in imaginary time, with weight $e^{-S_E}$.
...
This is often described as a **ring polymer** of $N$ beads, where the kinetic term couples nearest neighbors $(i,i+1)$, and the potential term acts on each bead individually.
...
$\Delta S$ is local. Only terms involving $x_{i-1},x_i,x_{i+1}$ change. Define:
...
\frac{1}{a}\text{arcosh}\left(\frac{G(\tau-a)+G(\tau+a)}{2G(\tau)}\right)
$$
...
This repository uses the harmonic case as a clean end-to-end validation of sampling, correlator estimation, and gap extraction.
